...
from .models import CouplingCoreError, ShallowCloneError
...
def open_repo(repo_path: Path) -> git.Repo:
    """Open a git repository, raising CouplingCoreError on invalid input
    and ShallowCloneError if fetch-depth: 0 was not configured.
    """
    try:
        repo = git.Repo(repo_path)
    except git.exc.InvalidGitRepositoryError as exc:
        raise CouplingCoreError(
            f"coupling-core: Error — not a git repository: {repo_path}"
        ) from exc
    except git.exc.NoSuchPathError as exc:
        raise CouplingCoreError(
            f"coupling-core: Error — path does not exist: {repo_path}"
        ) from exc

    if repo.git.rev_parse("--is-shallow-repository").strip() == "true":
        raise ShallowCloneError(
            "coupling-core: Error — repository is a shallow clone. "
...
def get_repo_name(repo: git.Repo) -> str:
    """Return the owner/repo identifier from the origin remote, falling back
    to the working-directory name when no remote is configured or the URL
    cannot be parsed.

    Supports both HTTPS-style (https://host/owner/repo[.git]) and SSH-style
    (git@host:owner/repo[.git], ssh://git@host/owner/repo[.git]) URLs. The
    last two path segments are returned, which gives owner/repo for GitHub
    and the deepest subgroup/repo for GitLab.
    """
    try:
        url = repo.git.remote("get-url", "origin").strip()
    except git.exc.GitCommandError:
        return Path(str(repo.working_dir)).name
